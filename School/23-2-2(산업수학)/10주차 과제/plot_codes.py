...
def plot_model(model, feature_labmda, xy, labels, title):
    ...
    cs = ax.contourf(xx, yy, Z, 20, cmap=plt.cm.jet, alpha=.8)
    ax.clabel(cs, colors='k')
    cs = ax.contour(xx, yy, Z, cmap=plt.cm.jet, levels=[0, 0.5], linestyles='--', linewidths=2)
    ax.clabel(cs, colors='k')
...
def plot_scatter_softmax(curr_W, curr_b, xy, labels):
    ...
    cs = plt.contourf(X, Y, Z, np.linspace(0, 1, 11), cmap=plt.cm.jet, alpha=.8)
